...
After executing this script, the counterfactual fairness of the base model should be determined.
...
## with code from https://stackoverflow.com/questions/65420136/how-do-you-adjust-the-arrows-in-a-ggplot-of-a-ggdag
```{r}
dag = dagitty('dag {
...
dag_plot = tidy_dagitty(dag) %>% 
  ...
  mutate(slope = (yend - y) / (xend - x), 
         ...
         proportion = shorten_distance/distance,
         xend = (1-proportion)*xend + (proportion*x), 
         yend = (1-proportion)*yend + (proportion*y)) %>% 
  ...
  geom_dag_node(data = function(x) subset(x, name %in% c("Straftaten", 
                ...
                color = colors[4],
                size = 25) +
  geom_dag_text(size = 3, family = "serif", parse = TRUE) +
...
dag_counter_plot = tidy_dagitty(dag_counter) %>% 
  ...
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
    geom_rrect(aes(xmin = 2.25, xmax = 4.25, ymin = -2.25, ymax = 0.25,
                 fill = colors[1],
                 color = colors[1]),
             alpha = 0.01,
             r = unit(0.8, "cm")) +
  ...
  geom_dag_node(data = function(x) subset(x, name %in% c("Straftaten", 
                ...
                color = colors[4],
                size = 18) +
  geom_dag_text(size = 2.2, family = "serif", parse = TRUE) +
  theme_dag(base_family = "serif") +
  guides(fill = "none",
         color = "none")
...
plot_sex_logit = pred_sex$plot_comp_logit +
   geom_label(x = 0.55, y = 1.25, label = "Original", color = bin_colors_sex[1,1], family = "serif") +
   geom_label(x = 0.2, y = 1.2, label = "Kontrafaktisch", color = bin_colors_sex[2,1], family = "serif") +
   theme(text = element_text(family = "serif"))

# Race
plot_race_wb_logit = pred_race_wb$plot_comp_logit +
   geom_label(x = 0.55, y = 1.25, label = "Original", color = bin_colors_race_wb[1,1], family = "serif") +
   geom_label(x = 0.2, y = 1.15, label = "Kontrafaktisch", color = bin_colors_race_wb[2,1], family = "serif") +
   theme(text = element_text(family = "serif"))

plot_race_wh_logit = pred_race_wh$plot_comp_logit +
   geom_label(x = 0.55, y = 1.3, label = "Original", color = bin_colors_race_wh[1,1], family = "serif") +
   geom_label(x = 0.1, y = 1.1, label = "Kontrafaktisch", color = bin_colors_race_wh[2,1], family = "serif") +
   theme(text = element_text(family = "serif")) 

plot_race_wo_logit = pred_race_wo$plot_comp_logit +
   geom_label(x = 0.55, y = 1.23, label = "Original", color = bin_colors_race_wo[1,1], family = "serif") +
   geom_label(x = 0.15, y = 1.1, label = "Kontrafaktisch", color = bin_colors_race_wo[2,1], family = "serif") +
   theme(text = element_text(family = "serif")) 
```
...
plot_theta_sex_base = data.frame(x = c("Original"),
  ...
  geom_text(aes(label = round(y, 4)), 
            vjust = -0.75,
            family = "serif") +
  labs(title = "(\u03B5-\u03B4)-approximative kontrafaktische Fairness für Geschlecht",
...
plot_theta_race_base = data.frame(x = c("Original"),
  ...
  geom_text(aes(label = round(y, 4)), 
            vjust = -0.75,
            family = "serif") +
  labs(title = "(\u03B5-\u03B4)-approximative kontrafaktische Fairness für Race",
       subtitle = "für Original-Modell (mit fixem \u03B5=0.1)",
       x = "Modell",
       y = "\u03B4") +
  scale_fill_manual(values = wes_palette("Darjeeling1")[c(5,3)]) +
