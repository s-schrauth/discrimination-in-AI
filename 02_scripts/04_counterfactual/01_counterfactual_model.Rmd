...
pred_sexf$weight_table
pred_sexm$weight_table
```
...
```{r}
pred_sexf$plot_comp_logit
pred_sexf$plot_comp_bin

pred_raceblack$plot_comp_logit
pred_raceblack$plot_comp_bin

pred_raceasian$plot_comp_logit
pred_raceasian$plot_comp_bin

pred_racehisp$plot_comp_logit
pred_racehisp$plot_comp_bin

pred_racenative$plot_comp_logit
pred_racenative$plot_comp_bin

pred_raceother$plot_comp_logit
pred_raceother$plot_comp_bin
```

Test: counterfactual model with variable swap
```{r}
data_sexswap = data_counter %>% 
  mutate(Sex_Male = ifelse(Sex_Male == 1, 0, 1),
         Sex_Female = ifelse(Sex_Female == 1, 0, 1))

model_sexswap = model_counter(data = data_sexswap,
                              u_d = u_d, 
                              u_j = u_j, 
                              iter = 2000, 
                              chains = 4, 
...
pred_sexswap = prediction_counter(params_df = model_sexswap$params, 
                                  data = data_sexswap, 
                                  seed = 89358379,
                                  color_nr = 3, 
                                  plot_text = "Intervention Sexswap", 
                                  plot_text_bin = "Counterfactual", 
                                  plot_org_logit = pred_base$plot_counter_logit, 
                                  plot_org_bin = pred_base$plot_counter_bin)

saveRDS(model_sexswap, 
        file = here("01_data", "02_data_processed", "04_counterfactual", 
                    "21_model_sexswap.rds")
        )

saveRDS(pred_sexswap, 
        file = here("01_data", "02_data_processed", "04_counterfactual", 
                    "22_pred_sexswap.rds")
        )
pred_sexswap = readRDS(file = here("01_data", "02_data_processed", "04_counterfactual", 
                    "22_pred_sexswap.rds")
        )

pred_sexswap$plot_comp_logit
pred_sexswap$plot_comp_bin

pred_base$weight_table
pred_sexswap$weight_table
pred_sexswap$plot_compas_weights
