...
plot_base_rel = data.frame(x = c("Fair", "Unfair"),
  ...
  ggplot(aes(x = x, y = y, fill = factor(1))) +
    geom_col() +
    geom_text(aes(label = percent(y, accuracy = 0.1)), vjust = -0.35) +
    labs(title = "Anteil an fairen Paar-Vergleichen",
         subtitle = "mit (individueller) Fairness als D(Mx, My) \u2264 d(x,y)",
         x = "",
...
plot_base_dens = diff_dist_ordered %>% 
  ggplot() +
    ...
    labs(title = "Differenz der Input- und Output-Unterschiede aller Daten-Paare",
         subtitle = "Histogramm und Dichte für D(Mx,My)-d(x,y)",
         x = "Differenz von D(Mx,My) und d(x,y)",
         y = "Dichte") 
```
...
plot_fair_dens = plot_base_dens +
    ...
    labs(title = "Differenz der Input- und Output-Unterschiede aller Daten-Paare",
         ...
         x = "Differenz von D(Mx,My) und d(x,y)",
         y = "Dichte") +
    geom_label(x = -820, y = 0.0035, label = "In-processed", color = wes_palette("Darjeeling1")[3]) +
    geom_label(x = -50, y = 0.002, label = "Original", color = wes_palette("Darjeeling1")[5]) 
```
